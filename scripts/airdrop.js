...
async function setAllocation() {
  ...
  //console.log(akyuDistribution);
  for(var i = 0;i< distribData.length;i++){

    try{
      let gPrice = 10000000000;
      console.log("Attempting to allocate 250 AKYUs to accounts:",distribData[i],"\n\n");
      let r = await akyuDistribution.airdropTokens(distribData[i],{from:accounts[0], gas:4500000, gasPrice: gPrice});
      console.log("---------- ---------- ---------- ----------");
  ...
  events.get(function(error, log) {
      ...
      for (var i=0; i<event_data.length;i++){
          //let tokens = event_data[i].args.value.toString(10);
      ...
      console.log(`A total of ${sumTokens} AKYU tokens have been distributed to ${sumAccounts} accounts so far.`);
      ...
      //console.log(`Run 'node scripts/verify_airdrop.js ${akyuDistribution.address} > scripts/data/review.csv' to get a log of all the accounts that were distributed the airdrop tokens.`)
