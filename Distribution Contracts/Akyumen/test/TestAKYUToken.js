...
contract('AkyuToken', function(accounts) {

  ////

  const DECIMALSFACTOR = new BigNumber('10').pow('0')

  const TOKEN_NAME = "Akyu";
  const TOKEN_SYMBOL = "AKYU";
  ...
  before(async() => {
        akyuToken = await AkyuToken.new(accounts[0],{from:accounts[0]});
    });

  describe("Token Basic Properties", async function () {

    it("Name", async function () {
      let tokenName = await akyuToken.name({from:accounts[0]});
      assert.equal(tokenName,TOKEN_NAME);
    });

    it("Symbol", async function () {
      let tokenSymbol = await akyuToken.symbol({from:accounts[0]});
      assert.equal(tokenSymbol,TOKEN_SYMBOL);
    });

    it("Decimals", async function () {
      let tokenDecimals = await akyuToken.decimals({from:accounts[0]});
      assert.equal(parseInt(tokenDecimals),TOKEN_DECIMALS);
    });

    it("Total Supply", async function () {
      let tokenTotalSupply = await akyuToken.totalSupply({from:accounts[0]});
  ...
  describe("Token Transfer Functions", async function () {
    ...
    it("should transfer from owner to another address", async function () {
      await akyuToken.transfer(accounts[1],1000,{from:accounts[0]});
      let account1Balance = await akyuToken.balanceOf(accounts[1],{from:accounts[0]});
      assert.equal(account1Balance,1000);
    });

    it('should FAIL to transfer to null address', async() => {
      try {
          await akyuToken.transfer(0,1000,{from:accounts[0]});
      } catch (error) {
    ...
    it('should FAIL to transfer more tokens than available', async() => {
      try {
          await akyuToken.transfer(accounts[1],TOTAL_SUPPLY +1,{from:accounts[0]});
      } catch (error) {
  ...
  describe("Token TransferFrom / Allowance Functions", async function () {
    ...
    it('should give an allowance of 9999 to another account', async() => {
      await akyuToken.approve(accounts[3],9999,{from:accounts[0]});
      let allowance = await akyuToken.allowance(accounts[0],accounts[3],{from:accounts[0]});
      assert.equal(allowance.toString(10),9999);
    });

    it('should transferFrom from allowance', async() => {
      await akyuToken.transferFrom(accounts[0],accounts[4],3333,{from:accounts[3]});
      let updatedAllowance = await akyuToken.allowance(accounts[0],accounts[3],{from:accounts[0]});
      assert.equal(updatedAllowance.toString(10),6666);

      let account4Balance = await akyuToken.balanceOf(accounts[4],{from:accounts[0]});
      assert.equal(account4Balance.toString(10),3333);
    });

    it('should increase allowance', async() => {
      await akyuToken.increaseApproval(accounts[5],100,{from:accounts[0]});
      let updatedAllowance = await akyuToken.allowance(accounts[0],accounts[5],{from:accounts[0]});
      assert.equal(updatedAllowance.toString(10),100);
    });

    it('should decrease allowance', async() => {
      let allowanceToDecrease = 50;
      let origAllowance = await akyuToken.allowance(accounts[0],accounts[5],{from:accounts[0]});
      await akyuToken.decreaseApproval(accounts[5],allowanceToDecrease,{from:accounts[0]});
      let updatedAllowance = await akyuToken.allowance(accounts[0],accounts[5],{from:accounts[0]});
    ...
    it('should FAIL to transferFrom to null address', async() => {
      try {
          await akyuToken.transferFrom(accounts[0],0,1,{from:accounts[3]});
      } catch (error) {
    ...
    it('should FAIL to transferFrom if _from has not enough balance', async() => {
      try {
          await akyuToken.transferFrom(accounts[0],accounts[5],TOTAL_SUPPLY +1,{from:accounts[3]});
      } catch (error) {
    ...
    it('should FAIL to transferFrom more than the allowance granted', async() => {
      try {
          await akyuToken.transferFrom(accounts[0],accounts[5],50000,{from:accounts[3]});
      } catch (error) {
          logError("✅   Tried to transferFrom without enough balance and failed");
          return true;
      }
