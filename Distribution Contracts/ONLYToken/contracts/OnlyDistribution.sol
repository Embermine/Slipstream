...
/**
 * @title ONLY token initial distribution
 *
 * @dev Distribute allocated tokens
 */
contract OnlyDistribution is Ownable {
    using SafeMath for uint256;

    OnlyToken public ONLY;

    uint256 private constant decimalFactor = 10**uint256(0);
    enum AllocationType { CREATOR, AIRDROP, ADVISOR, PROMO }
    uint256 public constant INITIAL_SUPPLY   = 1000 * decimalFactor;
    uint256 public AVAILABLE_TOTAL_SUPPLY    = 1000 * decimalFactor;
    uint256 public AVAILABLE_CREATOR_SUPPLY  = 0 * decimalFactor; // 100% Released at Token Distribution (TD)
    uint256 public AVAILABLE_AIRDROP_SUPPLY  = 1000 * decimalFactor; // 100% released at token Distribution (TD)
    uint256 public AVAILABLE_ADVISOR_SUPPLY  = 0 * decimalFactor; // 100% Released at TD
    uint256 public AVAILABLE_PROMO_SUPPLY    = 0 * decimalFactor; // 100% Released at TD
  ...
  // Keeps track of whether or not a ONLY airdrop has been made to a particular address
    ...
    event LogOnlyClaimed(address indexed _recipient, uint8 indexed _fromSupply, uint256 _amountClaimed, uint256 _totalAllocated, uint256 _grandTotalClaimed);

  /**
    * @dev Constructor function - Set the ONLY token address
    * @param _startTime The time when OnlyDistribution goes live
    */
    function OnlyDistribution(uint256 _startTime) public {
        ...
        ONLY = new OnlyToken(this);
    }

  /**
    * @dev Allow the owner of the contract to assign a new allocation
    * @param _recipient The recipient of the allocation
    * @param _totalAllocated The total amount of ONLY available to the receipient (after vesting)
    * @param _supply The ONLY supply the allocation will be taken from
    */
    ...
    */
        ...
        for(uint256 i = 0; i<_recipient.length; i++)
    {
            if (!airdrops[_recipient[i]]) {
                ...
                require(ONLY.transfer(_recipient[i], 2 * decimalFactor));
                airdropped = airdropped.add(2 * decimalFactor);
        }
        }
    ...
    */
    function transferTokens (address _recipient) public {
      ...
      // Transfer total allocated (minus previously claimed tokens)
        ...
        require(ONLY.transfer(_recipient, tokensToTransfer));
        grandTotalClaimed = grandTotalClaimed.add(tokensToTransfer);
        emit LogOnlyClaimed(_recipient, allocations[_recipient].AllocationSupply, tokensToTransfer, newAmountClaimed, grandTotalClaimed);
    }

  // Returns the amount of ONLY allocated
