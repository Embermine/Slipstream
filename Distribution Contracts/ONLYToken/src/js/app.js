...
App = {
  ...
  initContract: function() {
    $.getJSON("OnlyDistribution.json", function(data) {
      // console.log("data", data);
      // Get the necessary contract artifact file and instantiate it with truffle-contract.
      var OnlyDistributionArtifact = data;
      App.contracts.OnlyDistribution = TruffleContract(
        OnlyDistributionArtifact
      );

      // Set the provider for our contract.
      App.contracts.OnlyDistribution.setProvider(App.web3Provider);

      if (http_provider == "https://ropsten.infura.io/" + INFURA_APIKEY) {
        appInstance = App.contracts.OnlyDistribution.at(ropsten_address);
        ...
        appInstance = App.contracts.OnlyDistribution.deployed();
  ...
  initONLYContract: function(contract_address) {
    console.log("initiate Only token");
    $.getJSON("OnlyToken.json", function(data) {
      // Get the necessary contract artifact file and instantiate it with truffle-contract.
      var OnlyTokenArtifact = data;
      // console.log(data);
      App.contracts.OnlyToken = TruffleContract(OnlyTokenArtifact);

      // Set the provider for our contract.
      App.contracts.OnlyToken.setProvider(App.web3Provider);

      var appInstance2;
      var onlyTokenInstance;

      if (http_provider == "https://ropsten.infura.io/" + INFURA_APIKEY) {
        appInstance2 = App.contracts.OnlyToken.at(contract_address);
        // console.log("aaabbbb", appInstance);
        ...
        appInstance2 = App.contracts.OnlyToken.deployed();
      }
      console.log(appInstance2);
      // return;

      // appInstance2.then(function(instance) {
      //   onlyTokenInstance = instance;
      //   console.log(instance);

      //   onlyTokenInstance.totalSupply().then(function(result) {
      //     console.log(result);
      //   });
      // });
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      appInstance
        .then(function(instance) {
          ...
          // console.log(onlyDistributionInstance.INITIAL_SUPPLY().call());

          onlyDistributionInstance.ONLY().then(function(result) {
            console.log(result);

            var onlyTokenInstance = result;

            App.initONLYContract(result);

            // appInstance = App.contracts.OnlyDistribution.at(result);

            // console.log(appInstance);
          });

          onlyDistributionInstance.INITIAL_SUPPLY().then(function(result) {
            ...
            onlyDistributionInstance.grandTotalClaimed().then(function(result) {
