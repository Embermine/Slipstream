...
async function distributeTokens() {
  ...
  `);
    ...
    for(var i = 0;i< allocData.length;i++){
        ...
        if(prevAllocation[3].toNumber() == 0){
          ...
          try{
            ...
            let receipt = await bongDistribution.transferTokens(allocData[i][0],{from:accounts[0], gas:200000, gasPrice: 10000000000});
            if(receipt && receipt.logs.length >0){
              ...
              console.log('\x1b[31m%s\x1b[0m',"Failed to distribute vested BONG tokens for account:",allocData[i][0]);
            }
    ...
    console.log('\x1b[32m%s\x1b[0m',"Successfully distributed",sumDistributed, "BONG tokens to ", sumAccountsDistributed,"accounts");
