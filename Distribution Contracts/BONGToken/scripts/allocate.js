...
async function setAllocation() {
  ...
  `);
    ...
    for(var i = 0;i< allocData.length;i++){
        ...
        if(prevAllocation[3].toNumber() ==0){
            try{
                ...
                let receipt = await bongDistribution.setAllocation(allocData[i][0],web3.utils.toWei(tokens.toString(10)),ALLOC_TYPE,{from:accounts[0], gas:200000, gasPrice: 10000000000});
                if(receipt && receipt.logs.length >0){
                    ...
                    console.log("Tried to allocate", tokens.toString(10), "BONG tokens for account:",allocData[i][0]);
    ...
    console.log('\x1b[32m%s\x1b[0m',"Successfully allocated",sumAllocations, "BONG tokens to ", sumAccountsAllocated,"accounts");
