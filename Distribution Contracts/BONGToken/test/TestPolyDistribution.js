const BongDistribution = artifacts.require("./BongDistribution.sol");
...
contract('BongDistribution', function(accounts) {

  let bongDistribution;
  let bongToken;
  let bongTokenAddress;
  ...
  function logWithdrawalData(_allocationType, _currentBlockTime, _account_presale, _contractStartTime, _allocation, _new_presale_tokenBalance){
    ...
    console.log("BONG token balance :", _new_presale_tokenBalance.toString(10));
    console.log("\n");
  }
  ...
  async function doAllocationTests(_allocationType, _tokenAllocation, _accountToUse) {
    it("should allocate "+ _allocationType +" tokens", async function () {
      ...
      switch (_allocationType) {
        case "PRESALE":
            oldPresaleSupply = await bongDistribution.AVAILABLE_PRESALE_SUPPLY({from:account_owner});
            allocationTypeNum = 0;
          break;
        case "FOUNDER":
            oldPresaleSupply = await bongDistribution.AVAILABLE_FOUNDER_SUPPLY({from:account_owner});
            allocationTypeNum = 1;
          break;
        case "ADVISOR":
            oldPresaleSupply = await bongDistribution.AVAILABLE_ADVISOR_SUPPLY({from:account_owner});
            allocationTypeNum = 3;
          break;
        case "RESERVE":
            oldPresaleSupply = await bongDistribution.AVAILABLE_RESERVE_SUPPLY({from:account_owner});
            allocationTypeNum = 4;
          break;
        case "BONUS1":
            oldPresaleSupply = await bongDistribution.AVAILABLE_BONUS1_SUPPLY({from:account_owner});
            allocationTypeNum = 5;
          break;
        case "BONUS2":
            oldPresaleSupply = await bongDistribution.AVAILABLE_BONUS2_SUPPLY({from:account_owner});
            allocationTypeNum = 6;
          break;
        case "BONUS3":
            oldPresaleSupply = await bongDistribution.AVAILABLE_BONUS3_SUPPLY({from:account_owner});
            allocationTypeNum = 7;
          break;
        default:
      ...
      await bongDistribution.setAllocation(accountToUse,tokenAllocation,allocationTypeNum,{from:account_owner});
      let allocation = await bongDistribution.allocations(accountToUse,{from:account_owner});
      ...
      switch (_allocationType) {
        case "PRESALE":
          newPresaleSupply = await bongDistribution.AVAILABLE_PRESALE_SUPPLY({from:account_owner});
          break;
        case "FOUNDER":
          newPresaleSupply = await bongDistribution.AVAILABLE_FOUNDER_SUPPLY({from:account_owner});
          break;
        case "ADVISOR":
          newPresaleSupply = await bongDistribution.AVAILABLE_ADVISOR_SUPPLY({from:account_owner});
          break;
        case "RESERVE":
          newPresaleSupply = await bongDistribution.AVAILABLE_RESERVE_SUPPLY({from:account_owner});
          break;
        case "BONUS1":
          newPresaleSupply = await bongDistribution.AVAILABLE_BONUS1_SUPPLY({from:account_owner});
          break;
        case "BONUS2":
          newPresaleSupply = await bongDistribution.AVAILABLE_BONUS2_SUPPLY({from:account_owner});
          break;
        case "BONUS3":
          newPresaleSupply = await bongDistribution.AVAILABLE_BONUS3_SUPPLY({from:account_owner});
          break;
        default:
  ...
  before(async() => {
        bongDistribution = await BongDistribution.new(_startTime,{from:accounts[0]});
        bongTokenAddress = await bongDistribution.BONG({from:accounts[0]});
        bongToken = await BongToken.at(bongTokenAddress);

        contractStartTime = await bongDistribution.startTime({from:accounts[0]});
    });
  ...
  describe("All tests", async function () {
    ...
    describe("Test Constructor", async function () {
      ...
      it("should have deployed BongToken", async function () {
        logTitle("BongToken Address: "+ bongTokenAddress);
        assert.notEqual(bongTokenAddress.valueOf(), "0x0000000000000000000000000000000000000000", "Token was not initialized");
      });
    ...
    describe("Allocations", async function () {
      ...
      before(async() => {
        oldTotalSupply = await bongDistribution.AVAILABLE_TOTAL_SUPPLY({from:account_owner});
      });
      ...
      describe("Allocation post tests", async function () {
        ...
        it("New total supply should match allocations previously made", async function () {
          ...
          let newTotalSupply = await bongDistribution.AVAILABLE_TOTAL_SUPPLY({from:account_owner});
        ...
        it("Grand total should match allocations previously made", async function () {
          ...
          let grandTotalAllocated = await bongDistribution.grandTotalAllocated({from:account_owner});
      ...
      describe("Allocation invalid parameters", async function () {
        ...
        it("should reject invalid _supply codes", async function () {
          try {
            await bongDistribution.setAllocation(account_advisor1,1000,8,{from:account_owner});
          } catch (error) {
        ...
        it("should reject invalid address", async function () {
          try {
            await bongDistribution.setAllocation(0,1000,0,{from:account_owner});
          } catch (error) {
        ...
        it("should reject invalid allocation", async function () {
          try {
            await bongDistribution.setAllocation(account_advisor1,0,0,{from:account_owner});
          } catch (error) {
        ...
        it("should reject repeated allocations", async function () {
          try {
            await bongDistribution.setAllocation(account_presale,1000,0,{from:account_owner});
          } catch (error) {
    ...
    describe("Withdrawal / transfer", async function () {
      ...
      describe("Withdraw immediately after allocations", async function () {
        ...
        it("should withdraw PRESALE tokens", async function () {
          ...
          let tokenBalance = await bongToken.balanceOf(account_presale,{from:accounts[0]});
          await bongDistribution.transferTokens(account_presale,{from:accounts[0]});
          let new_tokenBalance = await bongToken.balanceOf(account_presale,{from:accounts[0]});

          //PRESALE tokens are completely distributed once allocated as they have no vesting period nor cliff
          let allocation = await bongDistribution.allocations(account_presale,{from:account_owner});
        ...
        it("should fail to withdraw FOUNDER tokens as cliff period not reached", async function () {
          ...
          try {
            await bongDistribution.transferTokens(account_founder1,{from:accounts[0]});
          } catch (error) {
              let currentBlock = await web3.eth.getBlock("latest");

              let new_tokenBalance = await bongToken.balanceOf(account_founder1,{from:accounts[0]});
              let allocation = await bongDistribution.allocations(account_founder1,{from:account_owner});
        ...
        it("should fail to withdraw ADVISOR tokens as cliff period not reached", async function () {
          ...
          try {
            await bongDistribution.transferTokens(account_advisor1,{from:accounts[0]});
          } catch (error) {
              let currentBlock = await web3.eth.getBlock("latest");

              let new_tokenBalance = await bongToken.balanceOf(account_advisor1,{from:accounts[0]});
              let allocation = await bongDistribution.allocations(account_advisor1,{from:account_owner});
        ...
        it("should fail to withdraw RESERVE tokens as cliff period not reached", async function () {
          ...
          try {
            await bongDistribution.transferTokens(account_reserve,{from:accounts[0]});
          } catch (error) {
              let currentBlock = await web3.eth.getBlock("latest");

              let new_tokenBalance = await bongToken.balanceOf(account_reserve,{from:accounts[0]});
              let allocation = await bongDistribution.allocations(account_reserve,{from:account_owner});
        ...
        it("should perform the AIRDROP for 50 accounts", async function () {
          await bongDistribution.airdropTokens(airdrop_massive,{from:accounts[0]});

        });

        it("airdrop accounts should have 250 BONG each", async function () {
          for (var i = 0; i< airdrop_massive.length; i++){
            let tokenBalance = await bongToken.balanceOf(airdrop_massive[i],{from:accounts[0]});
        ...
        it("should set another admin for airdrop", async function () {
          await bongDistribution.setAirdropAdmin(account_admin1,true,{from:accounts[0]});

        });

        it("should perform the AIRDROP for 50 accounts with an admin", async function () {
          await bongDistribution.airdropTokens(airdrop_massive2,{from:account_admin1});

        });

        it("airdrop accounts should have 250 BONG each", async function () {
          for (var i = 0; i< airdrop_massive2.length; i++){
            let tokenBalance = await bongToken.balanceOf(airdrop_massive2[i],{from:accounts[0]});
            assert.equal(tokenBalance.toString(10), "250000000000000000000");
      ...
      describe("Withdraw 8 months after allocations", async function () {
        ...
        it("should withdraw RESERVE tokens", async function () {
          ...
          // Check token balance for account before calling transferTokens, then check afterwards.
          let tokenBalance = await bongToken.balanceOf(account_reserve,{from:accounts[0]});
          await bongDistribution.transferTokens(account_reserve,{from:accounts[0]});
          let new_tokenBalance = await bongToken.balanceOf(account_reserve,{from:accounts[0]});
      ...
      describe("Withdraw 15 months after allocations", async function () {
        ...
        it("should withdraw FOUNDER tokens", async function () {
          ...
          // Check token balance for account before calling transferTokens, then check afterwards.
          let tokenBalance = await bongToken.balanceOf(account_founder1,{from:accounts[0]});
          await bongDistribution.transferTokens(account_founder1,{from:accounts[0]});
          let new_tokenBalance = await bongToken.balanceOf(account_founder1,{from:accounts[0]});
        ...
        it("should withdraw BONUS 1 tokens", async function () {
          ...
          // Check token balance for account before calling transferTokens, then check afterwards.
          let tokenBalance = await bongToken.balanceOf(account_bonus1,{from:accounts[0]});
          await bongDistribution.transferTokens(account_bonus1,{from:accounts[0]});
          let new_tokenBalance = await bongToken.balanceOf(account_bonus1,{from:accounts[0]});

          //PRESALE tokens are completely distributed once allocated as they have no vesting period nor cliff
          let allocation = await bongDistribution.allocations(account_bonus1,{from:account_owner});
        ...
        it("should withdraw ADVISOR tokens", async function () {
          ...
          // Check token balance for account before calling transferTokens, then check afterwards.
          let tokenBalance = await bongToken.balanceOf(account_advisor1,{from:accounts[0]});
          await bongDistribution.transferTokens(account_advisor1,{from:accounts[0]});
          let new_tokenBalance = await bongToken.balanceOf(account_advisor1,{from:accounts[0]});
    ...
    describe("Ether Transfers", async function () {
      ...
      it("should reject transfers", async function () {
        try {
          await bongDistribution.sendTransaction({from:accounts[0], value:web3.utils.toWei("1","ether")});
        } catch (error) {
